...
def _send_2fa_code(user_rec: dict, method: str, code: str) -> str:
    if method == "email":
        ...
        if not to_email:
            raise RuntimeError("No email on record for this user.")
    ...
    if not to_msisdn:
        raise RuntimeError("No mobile number on record for this user.")
    ...
    if not api_key or not api_secret:
        raise RuntimeError("Missing VONAGE_API_KEY / VONAGE_API_SECRET.")
...
def render(go):
    ...
    if st.session_state["signin_stage"] == "credentials":
        ...
        if submitted:
            try:
                users = list_users(supabase, bucket, users_object)
            except Exception as e:
                st.error(f"Could not read Users.json: {e}")
                users = []
            user_rec = find_user_by_username(users, username)
            if not user_rec:
                st.error("Unknown username."); st.stop()
            stored_hash = user_rec.get("password_sha256")
            if not stored_hash:
                st.error("This account has no password set yet."); st.stop()
            ...
            try:
                ...
                st.session_state["pending_2fa"] = {"username": user_rec.get("username"),"method": method,"code": code,"contact": user_rec.get("email") if method == "email" else user_rec.get("mobile"),"user": user_rec}
                st.session_state["signin_stage"] = "verify"
                st.success(f"Code sent via {sent_to}."); st.rerun()
            except Exception as e:
